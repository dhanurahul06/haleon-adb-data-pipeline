...
# Read Silver table with error handling
try:
    silver_df = spark.table("rahul_de_databricks_dev.sanitized.pharma_sales")
    print(f"Successfully read Silver table, {silver_df.count()} rows")
except Exception as e:
    print(f"ERROR: Failed to read Silver table: {e}")
    raise

# Aggregate and write to Gold with error handling
try:
    ...
    print(f"ERROR: Failed to write Gold table: {e}")
    raise
